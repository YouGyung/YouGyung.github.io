...
> <br/> [레포 이동하기](https://github.com/Open-Eye-Im-Developer/MoGakGo-FE)

![327285487-b2bf40fd-9809-46a9-80e3-7e355e54af4d (1)](https://github.com/user-attachments/assets/f0dab997-b05f-40e8-b842-1e9a883aab46)

모각GO는 매칭 서비스인만큼, 실시간으로 매칭 요청에 대한 결과를 사용자가 확인하는 기능이 중요하다고 생각합니다. 따라서 일반적인 client와 server의 상호작용에서는 client가 server로 http Protocol을 통해 요청을 보내고, 서버의 응답을 받아서 사용자에게 노출하지만, server에서 client로 먼저 알림 정보를 전송하는 역 방향으로 동작하는 방식이 필요하다는 생각을 하게 되었습니다.
...
아래 작업 내역의 경우에는 테스트로 생성한 Firebase app을 기준으로 정리한 내용이며, 현재 삭제된 프로젝트이므로 모각GO와는 관련없는 정보입니다!
...
> 모각GO는 서비스 특성 상, 모바일로 사용할 가능성이 높기때문에 PWA로 구현을 계획했어요
...
2. 명확한 이유를 찾고 싶어서 기본적으로 모바일 기기에서는 safari에 대한 기본 notification 설정이 false로 되어져 있는데( 이 경우 aleart가 작동하지 않는다 ! ),
...
3. iOS에서는 웹 앱이 홈 화면에 설치되지 않은 경우 항상 `denied`를 반환하는데, 여러번의 배포 과정을 통해 **혹시 내가 PWA가 아닌가..?** 라는 질문을 하게 되었고 검증을 위해 LightHouse의 progresive web app을 통해 검사를 진행했다. → 만족하지 않은 조건이 있길래 만족시킨 후, 다시 테스트를 진행했다

4. ㅋ.ㅋ pwa가 아니었던.. 머슥.. 모바일에서 정상적으로 오는 것도 확인했움 !
...
<img width="50%" alt="스크린샷 2024-07-25 오후 3 18 24" src="https://github.com/user-attachments/assets/fb7d0e46-5b42-4011-8c83-55cb87c94914">
